...
models** after entering its identifier, Base URL, optional custom
...
and display name. Context window is negotiated between the client and the
upstream; it is not configured on the provider model. For Claude 1M context,
append the `[1m]` suffix to the model ID, for example `claude-opus-5[1m]`.
...
when one upstream exposes both GPT and Claude models on different APIs. Select
one model as the default and choose its protocol before saving, or add a model
manually when the upstream does not expose a model list.
