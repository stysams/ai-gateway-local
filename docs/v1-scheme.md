...
- 默认监听回环地址；可以显式配置 `listen.host: 0.0.0.0` 让数据面服务局域网客户端，但管理 API 仍只接受回环来源；端口冲突时失败，不自动换端口。
...
- 远程访问管理 API；第二期允许为数据面显式监听 `0.0.0.0`，但管理 API 仍只接受回环来源。
...
- `gateway.lock` 用于无头网关单实例互斥。
- 桌面控制台另有独立的进程级单实例互斥，与 `gateway.lock` 不是同一把锁。
...
- `host` 可选，缺省为 `127.0.0.1`，只允许 `127.0.0.1` 或 `0.0.0.0`。它控制共享监听器；管理 API 的访问边界不随该字段放宽。
...
- `global`、`per_client`、`per_provider`：数据面活动请求上限，整数范围 `0..1024`，默认 `0`（不限制）。`0` 只表示对应层关闭限制，不会覆盖其他层的显式限制。
- `stream_idle_seconds`：流式响应连续无上游数据的超时，范围 `1..86400` 秒，默认 `300`。它只限制空闲间隔，不限制持续有数据的流式请求总时长。
- `request_body_bytes`：单个数据面请求体上限，范围 `0..134217728` 字节，默认 `0`（使用既有的 128 MiB 合同）。
- `request_header_bytes`：单个数据面请求头总大小上限，范围 `0..1048576` 字节，默认 `0`（不额外限制）。达到上限返回 431，拒绝路径不得接触上游。
- `client_rate_per_minute`：单客户端固定一分钟窗口请求数上限，范围 `0..100000`，默认 `0`（不限制）。超出时立即返回 429 和 `client_rate_limit`，不得等待或调用上游。
- 请求解析出客户端和 provider 后，网关必须以非阻塞方式同时尝试取得全局、客户端和 provider 槽位。任一上限已满时立即返回 `429`，不得在网关内部排队，也不得调用上游。
...
数据面和只读管理路径可以读取成功写入后原子发布的不可变配置视图，不得修改该视图。需要构造新配置的写事务必须从深复制快照开始，再按上述流程校验、原子替换和发布；一次写入失败不得改变当前已发布视图。
...
- 并发限额达到上限时返回 429 和 `concurrency_limit`，拒绝路径不得接触上游。
- 请求体超过 `request_body_bytes` 时返回 413；请求头超过 `request_header_bytes` 时返回 431；客户端一分钟窗口超过 `client_rate_per_minute` 时返回 429。
- 无法解析的 JSON 返回 400。
- 未知协议路径返回 404。
- 客户端前缀非法返回 404。
- 数据面不对回环请求鉴权；传入的占位 API key 被忽略。管理面无论监听地址如何配置，都只接受 `RemoteAddr` 解析为 IPv4 或 IPv6 回环地址的请求；不信任 `X-Forwarded-For` 等转发头，非回环管理请求返回 403，且不得触发任何管理副作用。
...
- 流式响应连续 `stream_idle_seconds` 秒没有上游数据时，网关关闭上游响应并结束该请求；该超时不得截断持续有数据的长推理流。
...
所有端点位于 `/api/v1`。管理 API 与数据面共用监听器，但只接受真实 TCP 回环来源；配置 `listen.host: 0.0.0.0` 时，远程客户端只能访问 `/healthz`、`/readyz` 和 `/v1/*` 数据面，不能读取或修改管理资源，也不能触发关闭。管理 API 不信任 `X-Forwarded-For` 等转发头。非回环管理请求返回 403 和 `management_loopback_required`。
...
`GET /api/v1/metrics` 返回本地运行计数和延迟百分位，不返回请求正文、查询参数或密钥。字段包括 `requests`、`success`、`failed`、`cancelled`、`upstream_errors`、`active_requests`、`active_log_sessions`，以及 `latency_ms` 和 `first_byte_ms` 下的 `p50`、`p95`。`providers` 按 provider 返回当前连续失败次数和熔断是否打开。

上游连接错误、超时和 `5xx` 会按 provider 累计；连续三次失败后进入短暂熔断，数据面返回 `503` 与 `provider_circuit_open`。上游 `429`、客户端错误和有副作用请求不会自动重试，也不会因为单次客户端错误打开熔断。
...
- 桌面在派生 `serve` 进程之前必须探测 `gateway.lock`。锁已被占用时只等待管理面就绪，不得再派生一个网关进程。
- 桌面控制台同一用户只允许一个进程。再次启动必须通知已有实例显示主窗口并立即退出，不得再创建一个托盘图标。`ai-gateway-desktop serve` 走无头网关入口，不受桌面单实例锁约束，仍受 `gateway.lock` 约束。
- 停止网关通过管理 API 完成。
- 关闭窗口不停止网关。
- 退出托盘不等于停止网关。
...
### 2026-08-17 复核：第三方请求需要可开关的客户端伪装

实验对象：本机供应商 `any`（`https://anyrouter.top`，adapter `anthropic`），模型
`claude-fable-5`。证据文件：

- 成功：`%USERPROFILE%\.ai-gateway\logs\2026-08-17\req_f3b436f9cbc9bd8aeace0afc.jsonl`
- 失败：`%USERPROFILE%\.ai-gateway\logs\2026-08-17\req_ade45770cb0ed44d2232596e.jsonl`
  与 `req_25483fde65408838db303158.jsonl`
- 同会话 400：`req_67a2e5d8324da61991a42697.jsonl`

观察：

- 唯一 200 来自 Claude Code（`/c/claude/v1/messages`），入站带完整
  `Anthropic-Beta`（含 `context-1m-2025-08-07`）。
- 最新失败来自第三方 generic（`User-Agent: Anthropic/Go 1.56.0`，
  `/v1/messages`），入站没有 `Anthropic-Beta`。该供应商当时靠手工
  `extra_headers` 补 Claude Code 身份头。
- `extra_headers` 作用于全部入站，包括已经是 Claude Code 的请求；桌面
  「应用预设」只是把头写进配置，不是按客户端分支的运行时开关。
- 身份头取值必须继续以本机 Claude Code 2.1.228 与 Codex CLI 0.147.0
  的核验结果为准，不得转发会话或安装标识。
...
### 2026-08-18 复核：出站协议绑定到模型

产品原因：同一上游（同一 `base_url` 与钥匙）可同时提供 GPT 与 Claude
模型，但补全接口不同。provider 级单一 `adapter` 无法同时正确转发两类
请求。
