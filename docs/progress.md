...
1. 完成 `G0-02`：在干净提交上重跑并冻结优化前基线。
2. 按依赖关系启动第一批任务：`SEC-01`、`RES-01`、`PERF-01`、`LOG-01`、`OBS-01`。
...
> 阅读 `docs/progress.md`、`docs/optimization-roadmap.md` 和 `docs/v1-scheme.md`。第一期已经带遗留问题验收，默认继续 `G0-02`：在干净提交上重跑并冻结优化前基线，然后按依赖启动第二期第一批任务。第 13 节遗留问题不阻塞主线，除非用户明确要求处理。不要升级 Wails，不要改变冻结协议来换取基准或测试通过。
