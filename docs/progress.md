# ai-gateway 当前版本进度与历史验收

> 状态：`0.1.0-rc1` 已冻结；功能主线已完成，不再启动其他规划；第一期保留五项真实验收遗留
> 文档日期：2026-08-25
> 当前版本：`0.1.0-rc1`（Windows amd64）
> 包内构建提交：`3bc053c898fcc834ec931bc8e0ac9cc734f6173c`
> 当前源码主线：`3bc053c898fcc834ec931bc8e0ac9cc734f6173c`
> 发布包：`dist/ai-gateway-0.1.0-rc1-windows-amd64.zip`，SHA-256 `EA5AF1C6DE608C236E53D60E6626F3AAD857C22A6387ADC0840FF4CA20D47E7D`
> 第一验收平台：Windows 11

本文记录当前版本的进度、权威边界、已落地能力和历史验收证据，**不是新的工程合同，也不产生后续版本规划**。

完整的用户可见能力和项目收尾门项见 [feature-checklist.md](feature-checklist.md)。
...
任务包 A 到 J 和第二期优化任务已经在当前源码主线完成。现有 `0.1.0-rc1` 发布包作为当前版本基准。2026-08-22，产品所有者根据当前实际使用效果，决定将第一期标记为**已验收（带遗留问题）**；该决定不再派生新的版本规划。
...
§19 已于 2026-08-22 执行一轮真实验收。原始二十项门禁没有全部通过，但核心数据面、三客户端工具调用、路由、图片、日志、用量、还原和进程行为已经满足当前使用需求。未完成和部分通过项保留原始证据，并转入本文第 13 节遗留清单；它们不改变当前版本范围，也不自动转为新的实现任务。
...
这些不是新的任务包。它们已经在当前源码主线实现；截至包内构建提交 `3bc053c` 的能力进入现有 `0.1.0-rc1` 包，后续仅保留文档元数据更新：
...
18. **入站 `Anthropic-Beta` 与 `extra_headers` 按令牌并集。** Claude Code
    ...
    禁止转发。
19. **供应商伪装客户端开关。** `disguise_client` 为 `claude`、`codex` 或 `pi`
    时，只给第三方 `generic` 入站套用已核验身份头。`claude` 在 Messages
    入站上还要补齐 `thinking: {type: adaptive}`（需启用 reasoning）和
    系统文本块的 `cache_control: {type: ephemeral}`。`pi` 使用本机 Pi 0.84.2
    供应商配置核验的 `User-Agent: Pi Agent/1.0`。不得改写 tools、
...
28. **Codex 与 Claude Code 辅助模型可独立选择。** 桌面「客户端」高级设置从
    已启用模型目录选择子代理模型和标题生成模型。Codex 按已核验的 spawn header
    与 shadow/helper 调用信号在数据面改写；Claude Code 把子代理模型写入
    Opus、Sonnet、Fable 槽，把标题模型写入 Haiku 与 Small Fast 槽。空值或
    失效选择跟随当前路由。实现信号来自本机 OpenCodex 2.15.1 复核，详见规格 §20。
...
`0.1.0-rc1` 是当前唯一版本基准。现有发布包的版本、包内构建提交和 SHA-256 以本文开头记录为准；后续文档元数据变更不自动生成新版本。

当前文档只用于说明现有版本、记录已完成能力和保留历史验收证据。除非用户明确提出新的版本任务，不启动新的主体功能、性能优化、协议调整或发布规划。
...
源码主线：`3bc053c898fcc834ec931bc8e0ac9cc734f6173c`

包内提交：`3bc053c898fcc834ec931bc8e0ac9cc734f6173c`
SHA-256：`EA5AF1C6DE608C236E53D60E6626F3AAD857C22A6387ADC0840FF4CA20D47E7D`
...
> 阅读 `docs/feature-checklist.md`、本文和 `docs/v1-scheme.md`，以 `0.1.0-rc1` 为唯一基准。功能主线已完成，不要重新打开已冻结协议，不要根据旧路线图启动新功能。只有用户明确要求时，才建立新的版本任务和相应验收范围。
...
这些问题属于当前版本的历史验收记录，不自动扩大版本范围。若其中任一问题在实际使用中造成阻断，必须由用户明确提出新的处理任务。
