...
const status = { version: "0.1.0", pid: 4242, listen: "127.0.0.1:12600", logging_enabled: true, logging_body_enabled: true, autostart_enabled: false, clients: { codex: { point_state: "pointed" }, claude: { point_state: "not_pointed" }, "claude-desktop": { point_state: "not_pointed", mcp_point_state: "not_pointed" }, grok: { point_state: "drifted" }, generic: { point_state: "unknown" } }, routes: { codex: { provider: "openrouter", model: "gpt-5" }, claude: { provider: "openrouter", model: "anthropic/claude-sonnet-4" }, "claude-desktop": { provider: "openrouter", model: "anthropic/claude-sonnet-4" }, grok: { provider: "deepseek", model: "deepseek-chat" }, generic: { provider: "openrouter", model: "gpt-5" } } };
...
test.beforeEach(async ({ page }) => {
  await page.route("http://127.0.0.1:12600/**", async (route) => {
    ...
    const match = path.match(/\/clients\/(codex|claude|claude-desktop|grok)$/); if (match) { const client = match[1] as "codex" | "claude" | "claude-desktop" | "grok"; return route.fulfill({ json: { client, point_state: status.clients[client].point_state, mcp_point_state: client === "claude-desktop" ? "not_pointed" : undefined, mcp_target: client === "claude-desktop" ? "C:/Users/test/Claude/claude_desktop_config.json" : undefined, target: `C:/Users/test/.${client}/config`, backup_available: client !== "claude", mcp_backup_available: client === "claude-desktop", ...(client === "codex" ? { remote_compaction: false } : {}) } }); }
...
test("client routes list every enabled model as provider/model id", async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name === "mobile-dark") {
    ...
    await expect(page.getByText("Full value: openrouter/anthropic/claude-sonnet-4", { exact: true }).first()).toBeVisible();
  }
