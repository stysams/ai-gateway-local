...
const adapters = ["openai-chat", "openai-responses", "anthropic"] as const;
...
function emptyModel(adapter: string): ProviderModel {
  return { id: "", name: "", adapter, context_window: 0, max_output_tokens: 0 };
}

function modelAdapter(model: Pick<ProviderModel, "adapter"> | undefined, fallback: string): string {
  return model?.adapter && (adapters as readonly string[]).includes(model.adapter) ? model.adapter : fallback;
}
...
function Providers({ providers, t, run, notify }: { providers: Provider[]; t: (key: MessageKey) => string; run: (op: () => Promise<unknown>, message?: string) => Promise<void>; notify: (kind: ToastKind, message: string) => void }) {
  ...
  const edit = (p: Provider) => {
    const models = (p.models?.length ? p.models : [{ id: p.default_model, name: "", context_window: 0, max_output_tokens: 0 }]).map((model) => ({ ...model, adapter: modelAdapter(model, p.adapter) }));
    ...
    try {
      ...
      setForm((current) => {
        ...
        const fetched = result.data.map((model) => {
          ...
          return { id: model.raw_id, name: saved?.name || model.display_name || "", adapter: modelAdapter(saved, current.adapter), context_window: saved?.context_window || model.context_window || 0, max_output_tokens: saved?.max_output_tokens || model.max_output_tokens || 0, enabled: saved?.enabled };
        });
  ...
  if (open) return <section className="provider-editor-page">
    ...
    <form id="provider-form" className="form-panel provider-form" onSubmit={submit} noValidate>
      ...
      <Field label={t("defaultAdapter")} error={errors.adapter}><select value={form.adapter} onChange={(e) => setForm({ ...form, adapter: e.target.value })}>{adapters.map((adapter) => <option key={adapter}>{adapter}</option>)}</select></Field>
