      ...
      <main className="main">
        ...
        {busy && !status ? <div className="loading"><RefreshCw className="spin" size={18} />{t("loading")}</div> : (
          <div className="content">
            ...
            {page === "logs" && config && <Logs items={logs} enabled={config.logging.enabled} body={config.logging.body !== false} t={t} run={run} notify={pushToast} />}
...
function Routes({ status, providers, t, run }: { status: Status; providers: Provider[]; t: (key: MessageKey) => string; run: (op: () => Promise<unknown>, message?: string) => Promise<void> }) {
  ...
  return <section><SectionHeader title={t("routes")} description={t("routesDescription")} />
    ...
    <div className="route-clients"><h3>{t("clientRoutes")}</h3><p className="muted">{t("clientRoutesDescription")}</p><div className="route-list">{allClients.map((client) => { const currentId = catalogId(draft[client]); const currentKnown = catalog.some((item) => item.id === currentId); return <div className="route-row" key={client}><div><strong className="mono">{client}</strong><small>/c/{client}/v1</small></div><label><span>{t("defaultSelectedModel")}</span><select className="mono" aria-label={`${client} ${t("defaultSelectedModel")}`} value={currentId} onChange={(e) => { const next = catalog.find((item) => item.id === e.target.value); if (next) setDraft({ ...draft, [client]: { provider: next.provider, model: next.model } }); }}>{!currentKnown && <option value={currentId}>{currentId}</option>}{catalog.map((item) => <option key={item.id} value={item.id}>{item.id}</option>)}</select></label><button className="secondary" onClick={() => void run(() => api.updateRoute(client, draft[client]), t("success"))}><Check size={16} />{t("apply")}</button></div>; })}</div></div>
  </section>;
}
...
function Logs({ items, enabled, body, t, run, notify }: { items: LogSummary[]; enabled: boolean; body: boolean; t: (key: MessageKey) => string; run: (op: () => Promise<unknown>, message?: string) => Promise<void>; notify: (kind: ToastKind, message: string) => void }) {
  ...
  return <section><SectionHeader title={t("requestLog")} description={enabled ? (body ? t("loggingWithBody") : t("loggingMetadataOnly")) : t("disabled")} action={<div className="header-switches"><label className="switch"><input type="checkbox" checked={enabled} onChange={(e) => void run(() => api.setLogging(e.target.checked))} aria-label={t("logging")} /><span /><b>{t("logging")}</b></label><label className="switch"><input type="checkbox" checked={enabled && body} disabled={!enabled} onChange={(e) => void run(() => api.setLoggingBody(e.target.checked))} aria-label={t("bodyLogging")} /><span /><b>{t("bodyLogging")}</b></label></div>} />
